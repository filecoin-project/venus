...
func TestPaychGetMergeAddFunds(t *testing.T) {
	...
	require.Equal(t, big.Add(addFundsAmt1, addFundsAmt2), addFundsMsg.Message.Value)
}
...
func TestPaychAvailableFunds(t *testing.T) {
	...
	require.EqualValues(t, big.Add(createAmt, addFundsAmt), av.ConfirmedAmt)
	require.EqualValues(t, 0, av.PendingAmt.Int64())
	require.EqualValues(t, 0, av.QueuedAmt.Int64())
	require.EqualValues(t, 0, av.VoucherReedeemedAmt.Int64())

	// Add some vouchers
	voucherAmt1 := big.NewInt(3)
	...
	require.EqualValues(t, big.Add(voucherAmt1, voucherAmt2), av.VoucherReedeemedAmt)
}
