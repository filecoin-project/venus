...
func newFundsReq(ctx context.Context, amt big.Int) *fundsReq {
...
func (m *mergedFundsReq) sum() big.Int {
	sum := big.NewInt(0)
	for _, r := range m.reqs {
		if r.isActive() {
...
// be attempted.
func (ca *channelAccessor) getPaych(ctx context.Context, amt big.Int) (address.Address, cid.Cid, error) {
...
func (ca *channelAccessor) processQueue(channelID string) (*api.ChannelAvailableFunds, error) {
	...
	if len(ca.fundsReqQueue) == 0 {
		return ca.currentAvailableFunds(channelID, big.NewInt(0))
	}
...
func (ca *channelAccessor) currentAvailableFunds(channelID string, queuedAmt big.Int) (*api.ChannelAvailableFunds, error) {
	if len(channelID) == 0 {
		return nil, nil
	}
	...
	if channelInfo.Channel != nil {
		...
		for _, ls := range laneStates {
			...
			totalRedeemed = big.Add(totalRedeemed, r)
		}
	}
...
func (ca *channelAccessor) createPaych(ctx context.Context, amt big.Int) (cid.Cid, error) {
