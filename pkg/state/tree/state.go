...
func VersionForNetwork(ver network.Version) (StateTreeVersion, error) {
	...
	case network.Version18, network.Version19, network.Version20, network.Version21,
		network.Version22, network.Version23, network.Version24, network.Version25,
		network.Version26, network.Version27, network.Version28, network.Version29:
...
func NewState(cst cbor.IpldStore, ver StateTreeVersion) (*State, error) {
	...
	case StateTreeVersion0:
		// info is undefined
	case StateTreeVersion1, StateTreeVersion2, StateTreeVersion3, StateTreeVersion4, StateTreeVersion5:
		var err error
	...
	default:
		return nil, fmt.Errorf("unsupported state tree version: %d", ver)
	}
	...
	case StateTreeVersion0:
		...
		if err != nil {
			return nil, fmt.Errorf("failed to create state tree: %v", err)
		}
		hamt = tree.Map
	...
	case StateTreeVersion3:
		...
		if err != nil {
			return nil, fmt.Errorf("failed to create state tree: %w", err)
		}
		hamt = tree.Map
	case StateTreeVersion4:
		...
		hamt = tree.Map
	case StateTreeVersion5:
		tree, err := builtintypes.NewTree(store)
		if err != nil {
			return nil, fmt.Errorf("failed to create state tree: %w", err)
		}
		hamt = tree.Map
...
func LoadState(ctx context.Context, cst cbor.IpldStore, c cid.Cid) (*State, error) {
	...
	case StateTreeVersion5:
		var tree *builtintypes.ActorTree
		tree, err = builtintypes.LoadTree(store, root.Actors)
		if tree != nil {
			hamt = tree.Map
		}
	default:
...
func (st *State) SetActor(ctx context.Context, addr ActorKey, act *types.Actor) error {
	...
	if err != nil {
		return fmt.Errorf("ID lookup failed: %v", err)
	}
	addr = iaddr
...
func (st *State) lookupIDinternal(addr address.Address) (address.Address, error) {
	...
	if !found || err != nil {
		return address.Undef, fmt.Errorf("getting init actor: %v", err)
	}

	ias, err := init_.Load(&AdtStore{st.Store}, act)
	if err != nil {
		return address.Undef, fmt.Errorf("loading init actor state: %v", err)
	}
	...
	if err != nil {
		return address.Undef, fmt.Errorf("resolve address %s: %w", addr, err) // ends with %w implements an Unwrap method
	}
	return a, err
}
...
func (st *State) GetActor(ctx context.Context, addr ActorKey) (*types.Actor, bool, error) {
	...
	if addr.Protocol() != address.ID {
		if addr, err = st.LookupID(addr); err != nil {
			...
			return nil, false, fmt.Errorf("address resolution: %v", err)
		}
	}
	...
	var act types.Actor
	var found bool
	if st.version <= StateTreeVersion4 {
	...
	if err != nil {
		return nil, false, fmt.Errorf("hamt find failed: %v", err)
	}
	if !found {
		return nil, false, nil
	}
...
func (st *State) DeleteActor(ctx context.Context, addr ActorKey) error {
	...
	if addr == address.Undef {
		return fmt.Errorf("DeleteActor called on undefined address")
	}

	iaddr, err := st.LookupID(addr)
	if err != nil {
		if errors.Is(err, types.ErrActorNotFound) {
			return fmt.Errorf("resolution lookup failed (%s): %v", addr, err)
		}
		return fmt.Errorf("address resolution: %v", err)
	}
...
func (st *State) Flush(ctx context.Context) (cid.Cid, error) {
	...
	if len(st.snaps.layers) != 1 {
		return cid.Undef, fmt.Errorf("tried to flush state tree with snapshots on the stack")
	}
	...
	if err != nil {
		return cid.Undef, fmt.Errorf("failed to flush state-tree hamt: %v", err)
	}
...
func (st *State) ForEach(f func(ActorKey, *types.Actor) error) error {
	...
	if st.version <= StateTreeVersion4 {
		...
		return st.root.ForEach(&act, func(k string) error {
			...
			if err != nil {
				return fmt.Errorf("invalid address (%x) found in state tree key: %w", []byte(k), err)
			}
...
func Diff(oldTree, newTree *State) (map[string]types.Actor, error) {
	...
	if err := newTree.root.ForEach(&ncval, func(k string) error {
		addr, err := address.NewFromBytes([]byte(k))
		if err != nil {
			return fmt.Errorf("address in state tree was not valid: %v", err)
		}
		...
		if newTree.version <= StateTreeVersion4 {
			var act types.ActorV4
