	...
	for msgIdx, em := range ems {
		for evIdx, ev := range em.Events() {
			...
			if !entryID.Valid {
				...
				if err != nil {
					return fmt.Errorf("get last row id: %w", err)
				}
				...
				if err != nil {
					return fmt.Errorf("exec restore event: %w", err)
				}
...
func (ei *EventIndex) PrefillFilter(ctx context.Context, f *EventFilter) error {
	...
	for q.Next() {
		...
		if row.id != currentID {
			...
			if err != nil {
				return fmt.Errorf("parse emitter addr: %w", err)
			}

			ce.TipSetKey, err = types.TipSetKeyFromBytes(row.tipsetKey)
			if err != nil {
				return fmt.Errorf("parse tipsetkey: %w", err)
			}

			ce.MsgCid, err = cid.Cast(row.messageCid)
			if err != nil {
				return fmt.Errorf("parse message cid: %w", err)
			}
		}
