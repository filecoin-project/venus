...
func makeTestMpool() (*MessagePool, *testMpoolAPI) {
	...
	mp, err := New(context.Background(), tma, nil, ds, config.DefaultForkUpgradeParam, config.DefaultMessagePoolParam, "test", nil, mockGetNtwkVersionfunc)
	if err != nil {
		panic(err)
	}
...
func TestMessageChains(t *testing.T) {
	...
	maxMessages := int(constants.BlockGasLimit / gasLimit)
	...
	tma.setBalanceRaw(a1, tbig.NewInt(300*gasLimit+1))
...
func TestMessageSelectionTrimmingGas(t *testing.T) {
	...
	nMessages := int((constants.BlockGasLimit / gasLimit) + 1)
	for i := 0; i < nMessages; i++ {
		bias := (nMessages - i) / 3
		m := makeTestMessage(w1, a1, a2, uint64(i), gasLimit, uint64(1+i%3+bias))
		mustAdd(t, mp, m)
	...
	expected := int(constants.BlockGasLimit / gasLimit)
...
func TestPriorityMessageSelection2(t *testing.T) {
	...
	nMessages := int(2 * constants.BlockGasLimit / gasLimit)
	for i := 0; i < nMessages; i++ {
		bias := (nMessages - i) / 3
		m := makeTestMessage(w1, a1, a2, uint64(i), gasLimit, uint64(1+i%3+bias))
		mustAdd(t, mp, m)
	...
	expectedMsgs := int(constants.BlockGasLimit / gasLimit)
...
func TestOptimalMessageSelection2(t *testing.T) {
	...
	nMessages := int(5 * constants.BlockGasLimit / gasLimit)
	for i := 0; i < nMessages; i++ {
		bias := (nMessages - i) / 3
...
func testCompetitiveMessageSelection(t *testing.T, rng *rand.Rand, getPremium func() uint64) (float64, float64, float64) {
	...
	logging.SetLogLevel("messagepool", "info") // nolint: errcheck
...
func TestGasReward(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(fmt.Sprintf("%v", test), func(t *testing.T) {
			...
			rew := mp.getGasReward(msg, tbig.NewInt(int64(test.BaseFee)))
...
readLoop:
	...
	block := tma.nextBlockWithHeight(UpgradeBreezeHeight + 10)
	ts := mkTipSet(block)
	tma.applyBlock(t, block)
	...
	tma.baseFee = tbig.NewInt(800_000_000)
	...
	// greedy first
	selected, err := mp.SelectMessages(ctx, ts, 1.0)
	if err != nil {
		t.Fatal(err)
	}
	...
	// high quality ticket
	selected, err = mp.SelectMessages(ctx, ts, .8)
	if err != nil {
		t.Fatal(err)
	}
