...
import (
	...
	"github.com/filecoin-project/venus/tools/conformance"
	"sync"
	"time"
...
type StateManagerAPI interface {
	...
	LoadActorTsk(ctx context.Context, addr address.Address, tsk block.TipSetKey) (*types.Actor, error)
	LookupID(ctx context.Context, addr address.Address, ts *block.TipSet) (address.Address, error)
...
type StateManager struct {
	...
	stateMigrations map[abi.ChainEpoch]fork.UpgradeFunc
	...
	newVM               func(vm.VmOption) (vm.Interpreter, error)
...
func NewStateManager(cs *chain.Store, us fork.UpgradeSchedule, fk fork.IFork, cState *cst.ChainStateReadWriter) *StateManager {
	sm, err := NewStateManagerWithUpgradeSchedule(cs, us, fk, cState)
	if err != nil {
...
func NewStateManagerWithUpgradeSchedule(cs *chain.Store, us fork.UpgradeSchedule, fk fork.IFork, cState *cst.ChainStateReadWriter) (*StateManager, error) {
	// If we have upgrades, make sure they're in-order and make sense.
	if err := us.Validate(); err != nil {
		return nil, err
	}

	stateMigrations := make(map[abi.ChainEpoch]fork.UpgradeFunc, len(us))
	...
	return &StateManager{
		...
		newVM:             vm.NewVM,
		ms:                chain.NewMessageStore(cs.Blockstore()),
		fork:              fk,
		cs:                cs,
		cState:            cState,
...
func (sm *StateManager) TipSetState(ctx context.Context, ts *block.TipSet) (st cid.Cid, rec cid.Cid, err error) {
...
func traceFunc(trace *[]*api.InvocResult) func(mcid cid.Cid, msg *types.UnsignedMessage, ret *vmcontext.Ret) error {
	timeNow := time.Now()
	return func(mcid cid.Cid, msg *types.UnsignedMessage, ret *vmcontext.Ret) error {
		ir := &api.InvocResult{
			MsgCid:         mcid,
			Msg:            msg,
			MsgRct:         &ret.Receipt,
		...
		if ret.Receipt.ExitCode != 0 {
			ir.Error = fmt.Sprintf("initializing worker account failed: exit code %d", ret.Receipt.ExitCode)
		}
		if &ret.OutPuts != nil {
...
func MakeMsgGasCost(msg *types.UnsignedMessage, ret *vm.Ret) api.MsgGasCost {
	cidTmp, _ := msg.Cid()
...
func (sm *StateManager) ExecutionTrace(ctx context.Context, ts *block.TipSet) (cid.Cid, []*api.InvocResult, error) {
...
type ExecCallback func(cid.Cid, *types.UnsignedMessage, *vmcontext.Ret) error

func (sm *StateManager) ApplyBlocks(ctx context.Context, parentEpoch abi.ChainEpoch, pstate cid.Cid, bms []block.BlockMessagesInfo, epoch abi.ChainEpoch, cb ExecCallback, baseFee abi.TokenAmount, ts *block.TipSet) (cid.Cid, cid.Cid, error) {
	makeVmWithBaseState := func(base cid.Cid, epoch abi.ChainEpoch, baseFee abi.TokenAmount) (vm.Interpreter, error) {
		/*	vmopt := &vm.VmOption{
				StateBase:      base,
				Epoch:          epoch,
				Rand:           r,
		...
		*/
		...
		faultChecker := slashing.NewFaultChecker(sm.cState, sm.fork)
		syscalls := vmsupport.NewSyscalls(faultChecker, ffiwrapper.ProofVerifier)

		var (
			caculator = chain.NewCirculatingSupplyCalculator(bs, sm.cState, mainNetParams.Network.ForkUpgradeParam)
			vmOption  = vm.VmOption{
				...
				NtwkVersionGetter: sm.fork.GetNtwkVersion,
				Rnd:               conformance.NewFixedRand(),
				BaseFee:           baseFee,
				Fork:              sm.fork,
				Epoch:             epoch,
		...
		return sm.newVM(vmOption)
	}

	vmi, err := makeVmWithBaseState(pstate, epoch, baseFee)
	if err != nil {
		return cid.Undef, cid.Undef, xerrors.Errorf("making vm: %w", err)
	}
	runCron := func(epoch abi.ChainEpoch) error {
		cronMsg := &types.UnsignedMessage{
			To:         cron.Address,
		...
		ret, err := vmi.ApplyImplicitMessage(cronMsg)
		if err != nil {
			return err
		}
		if cb != nil {
			...
			if err := cb(cid, cronMsg, ret); err != nil {
				return xerrors.Errorf("callback failed on cron message: %w", err)
			}
		}
		if ret.Receipt.ExitCode != 0 {
			return xerrors.Errorf("CheckProofSubmissions exit was non-zero: %d", ret.Receipt.ExitCode)
		}
	...
	for i := parentEpoch; i < epoch; i++ {
		...
		// handle state forks
		// XXX: The state tree
		newState, err := sm.fork.HandleStateForks(ctx, pstate, i, ts)
		if err != nil {
			return cid.Undef, cid.Undef, xerrors.Errorf("error handling state forks: %w", err)
		}

		if pstate != newState {
			//TODO 加了2个参数，要确定是否是这两个参数
			vmi, err = makeVmWithBaseState(newState, i, baseFee)
			if err != nil {
				return cid.Undef, cid.Undef, xerrors.Errorf("making vm: %w", err)
			}
		}
	...
	for _, b := range bms {
		...
		for _, cm := range append(b.BlsMessages, b.SecpkMessages...) {
			...
			if _, found := processedMsgs[cidTmp]; found {
				continue
			}
			r, err := vmi.ApplyMessage(cm)
			if err != nil {
				return cid.Undef, cid.Undef, err
			}
			...
			gasReward = big.Add(gasReward, r.OutPuts.MinerTip)
			penalty = big.Add(penalty, r.OutPuts.MinerPenalty)

			if cb != nil {
				if err := cb(cmIdTmo, m, r); err != nil {
					return cid.Undef, cid.Undef, err
				}
			}
			processedMsgs[cidTmp] = struct{}{}
		}
		buf := bytes.NewBuffer(nil)
		err = (&reward.AwardBlockRewardParams{
			Miner:     b.Block.Miner,
			Penalty:   penalty,
			GasReward: gasReward,
			WinCount:  b.Block.ElectionProof.WinCount,
		}).MarshalCBOR(buf)
		if err != nil {
			return cid.Undef, cid.Undef, xerrors.Errorf("failed to serialize award params: %w", err)
		}

		rwMsg := &types.UnsignedMessage{
			From:       builtin.SystemActorAddr,
			To:         reward.Address,
			Nonce:      uint64(epoch),
			Value:      big.NewInt(0),
		...
		rwMsgCid, err := rwMsg.Cid()
		return cid.Undef, cid.Undef, err
		ret, actErr := vmi.ApplyImplicitMessage(rwMsg)
		if actErr != nil {
			return cid.Undef, cid.Undef, xerrors.Errorf("failed to apply reward message for miner %s: %w", b.Block.Miner, actErr)
		}
		if cb != nil {
			if err := cb(rwMsgCid, rwMsg, ret); err != nil {
				return cid.Undef, cid.Undef, xerrors.Errorf("callback failed on reward message: %w", err)
			}
		}
		if ret.Receipt.ExitCode != 0 {
			return cid.Undef, cid.Undef, xerrors.Errorf("reward application message failed (exit %d)", ret.Receipt.ExitCode)
		}
	}
	...
	rectarr, err := adt.NewArray(adt.WrapStore(ctx, sm.cs.ReadOnlyStateStore().IpldStore), specactors.VersionForNetwork(sm.GetNtwkVersion(ctx, epoch)))
	if err != nil {
...
func (sm *StateManager) computeTipSetState(ctx context.Context, ts *block.TipSet, cb ExecCallback) (cid.Cid, cid.Cid, error) {
	...
	if blks[0].Height > 0 {
		parent, err := sm.cs.GetBlock(ctx, blks[0].Parents.Cids()[0])
		if err != nil {
	...
	blkmsgs, err := sm.ms.LoadTipSetMessage(ctx, ts)
	if err != nil {
		return cid.Undef, cid.Undef, xerrors.Errorf("getting block messages for tipset: %w", err)
	}
	baseFee := blks[0].ParentBaseFee
	return sm.ApplyBlocks(ctx, parentEpoch, pstate, blkmsgs, blks[0].Height, cb, baseFee, ts)
}
...
func (sm *StateManager) ChainStore() *chain.Store {
	return sm.cs
}
...
func (sm *StateManager) ResolveToKeyAddress(ctx context.Context, addr address.Address, ts *block.TipSet) (address.Address, error) {
...
func (sm *StateManager) GetBlsPublicKey(ctx context.Context, addr address.Address, ts *block.TipSet) (pubk []byte, err error) {
...
func (sm *StateManager) LookupID(ctx context.Context, addr address.Address, ts *block.TipSet) (address.Address, error) {
	cst := cbor.NewCborStore(sm.cs.Blockstore())
...
func (sm *StateManager) GetReceipt(ctx context.Context, msg cid.Cid, ts *block.TipSet) (*types.MessageReceipt, error) {
...
func (sm *StateManager) WaitForMessage(ctx context.Context, mcid cid.Cid, confidence uint64, lookbackLimit abi.ChainEpoch) (*block.TipSet, *types.MessageReceipt, cid.Cid, error) {
	...
	reverts := map[block.TipSetKey]bool{}
...
func (sm *StateManager) SearchForMessage(ctx context.Context, mcid cid.Cid) (*block.TipSet, *types.MessageReceipt, cid.Cid, error) {
	msg, err := sm.cs.GetCMessage(mcid)
	if err != nil {
		return nil, nil, cid.Undef, fmt.Errorf("failed to load message: %w", err)
	}
...
func (sm *StateManager) searchBackForMsg(ctx context.Context, from *block.TipSet, m types.ChainMsg, limit abi.ChainEpoch) (*block.TipSet, *types.MessageReceipt, cid.Cid, error) {
...
func (sm *StateManager) tipsetExecutedMessage(ts *block.TipSet, msg cid.Cid, vmm *types.UnsignedMessage) (*types.MessageReceipt, cid.Cid, error) {
...
func (sm *StateManager) MarketBalance(ctx context.Context, addr address.Address, ts *block.TipSet) (api.MarketBalance, error) {
...
func (sm *StateManager) ValidateChain(ctx context.Context, ts *block.TipSet) error {
	tschain := []*block.TipSet{ts}
	for ts.Height() != 0 {
...
func (sm *StateManager) setupPostIgnitionVesting(ctx context.Context) error {
	...
	for k, v := range totalsByEpoch {
		ns := msig0.State{
			...
			StartEpoch: config.DefaultForkUpgradeParam.UpgradeLiftoffHeight,
		}
...
func (sm *StateManager) GetFilVested(ctx context.Context, height abi.ChainEpoch, st *state.StateTree) (abi.TokenAmount, error) {
	vf := big.Zero()
	if height <= config.DefaultForkUpgradeParam.UpgradeIgnitionHeight {
		for _, v := range sm.preIgnitionVesting {
			au := big.Sub(v.InitialBalance, v.AmountLocked(height))
			vf = big.Add(vf, au)
		}
	} else if height <= config.DefaultForkUpgradeParam.UpgradeCalicoHeight {
	...
	if height <= config.DefaultForkUpgradeParam.UpgradeActorsV2Height {
