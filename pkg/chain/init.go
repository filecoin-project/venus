...
func Init(ctx context.Context, r repo.Repo, bs bstore.Blockstore, cst cbor.IpldStore, gen genesis.InitFunc) (*Store, error) {
	...
	chainStore := NewStore(r.ChainDatastore(), cst, bs, NewStatusReporter(), nil, genesis.Cid())

	// Persist the genesis tipset to the repo.
	genTsas := &TipSetMetadata{
		TipSet:          genTipSet,
		TipSetStateRoot: genesis.ParentStateRoot.Cid,
		TipSetReceipts:  genesis.ParentMessageReceipts.Cid,
	}
