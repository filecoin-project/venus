...
func NewDefaultSyncer(cst *hamt.CborIpldStore, c consensus.Protocol, s syncerChainReader, f syncFetcher, blkTime time.Duration) *DefaultSyncer {
...
func (syncer *DefaultSyncer) syncOne(ctx context.Context, parent, next types.TipSet) error {
	...
	pMinTs, err := parent.MinTimestamp()
	if err != nil {
		return err
	}

	tickets, err := next.Tickets()
	if err != nil {
		return err
	}

	for _, b := range next.ToSlice() {
		if uint64(b.Timestamp) > uint64(time.Now().UTC().Unix()) {
		...
		// By setting this to strictly less than, we allow for a 1 second drift
		// in clocks. This has an added benefit in that it keeps FAST tests
		// working that mine at the sub-second level.
		// The loss of precision in seconds need not matter.
