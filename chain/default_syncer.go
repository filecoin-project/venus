...
type DefaultSyncer struct {
	...
	validator consensus.BlockSemanticValidator
}

var _ Syncer = (*DefaultSyncer)(nil)

// NewDefaultSyncer constructs a DefaultSyncer ready for use.
func NewDefaultSyncer(cst *hamt.CborIpldStore, c consensus.Protocol, v consensus.BlockValidator, s syncerChainReader, f syncFetcher) *DefaultSyncer {
...
func (syncer *DefaultSyncer) syncOne(ctx context.Context, parent, next types.TipSet) error {
	...
	if err := syncer.validator.ValidateSemantics(ctx, next, parent); err != nil {
		return err
	}
