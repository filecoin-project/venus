...
func TestCollectTipSetsOfHeightAtLeast(t *testing.T) {
	...
	tipsets, err := chain.CollectTipSetsOfHeightAtLeast(ctx, chainStore, &headTipSetAndState.TipSet, stopHeight)
	assert.NoError(err)
...
func TestCollectAtMostNTipSets(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		...
		require.NoError(err)
		number := uint(10)
		tipsets, err := chain.CollectAtMostNTipSets(ctx, chainStore, &headTipSetAndState.TipSet, number)
		assert.NoError(err)
