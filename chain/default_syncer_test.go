...
func loadSyncerFromRepo(t *testing.T, r repo.Repo, dstP *DefaultSyncerTestParams) (*chain.DefaultSyncer, *th.TestFetcher) {
	...
	con := consensus.NewExpected(cst, bs, th.NewTestProcessor(), th.NewTestBlockValidator(), powerTable, dstP.genCid, verifier)
...
func initSyncTestDefault(t *testing.T, dstP *DefaultSyncerTestParams) (*chain.DefaultSyncer, *chain.Store, repo.Repo, *th.TestFetcher) {
	...
	cst := hamt.NewCborStore()
	verifier := proofs.NewFakeVerifier(true, nil)
	con := consensus.NewExpected(cst, bs, processor, th.NewTestBlockValidator(), powerTable, dstP.genCid, verifier)
...
func TestTipSetWeightDeep(t *testing.T) {
	...
	verifier := proofs.NewFakeVerifier(true, nil)
	con := consensus.NewExpected(cst, bs, th.NewTestProcessor(), th.NewTestBlockValidator(), &th.TestView{}, calcGenBlk.Cid(), verifier)
	...
	verifier = proofs.NewFakeVerifier(true, nil)
	con = consensus.NewExpected(cst, bs, th.NewTestProcessor(), th.NewTestBlockValidator(), &consensus.MarketView{}, calcGenBlk.Cid(), verifier)
