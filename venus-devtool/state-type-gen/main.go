...
func run(cctx *cli.Context) error {
	for _, m := range metas {
		pkgInfos := make(map[string]*pkgInfo, len(m.includePkgs))
...
func outputFile(dst string, opt outputOpt, pkgInfos map[string]*pkgInfo, fopt option) error {
	if opt.typ == 0 {
		return writePkgInfo(filepath.Join(dst, opt.fileName), pkgInfos, fopt)
	}

	for name, pi := range pkgInfos {
		for _, fi := range pi.files {
			...
			genFileDetail(&fileBuffer, fi, name, fopt)
...
func writePkgInfo(dst string, pkgInfos map[string]*pkgInfo, fopt option) error {
	var fileBuffer bytes.Buffer
	fmt.Fprintf(&fileBuffer, "// Code generated by github.com/filecoin-project/venus/venus-devtool/state-type-gen. DO NOT EDIT.\npackage %s\n\n", "types")
	...
	for name, pi := range pkgInfos {
		...
		genFileDetail(&fileBuffer, &merge, name, fopt)
		fmt.Fprintln(&fileBuffer, "\n")
	}
