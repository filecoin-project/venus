...
func LoadGenesis(rep repo.Repo, sourceName string) (cid.Cid, error) {
	...
	if err != nil {
		return cid.Cid{}, fmt.Errorf("invalid filepath or URL fort genesis file: %s", sourceURL)
	}
	if sourceURL.Scheme == "http" || sourceURL.Scheme == "https" {
		...
		return cid.Cid{}, fmt.Errorf("unsupported protocol for genesis file: %s", sourceURL.Scheme)
	} else {
	...
	if len(ch.Roots) != 1 {
		return cid.Cid{}, fmt.Errorf("expected car with only a single root")
	}
