...
type Fetcher interface {
	FetchTipSets(ctx context.Context, tsKey types.TipSet, recur int) ([]types.TipSet, error)
}
...
func (bsf *BitswapFetcher) FetchTipSets(ctx context.Context, tsKey types.TipSet, recur int) ([]types.TipSet, error) {
	var out []types.TipSet
	cur := tsKey.ToSortedCidSet()
	for i := 0; i < recur; i++ {
