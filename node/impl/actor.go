...
func ls(ctx context.Context, fcn *node.Node, actorGetter state.GetAllActorsFunc) ([]*iface.ActorView, error) {
	...
	res := make([]*iface.ActorView, len(actors))
...
func makeActorView(act *types.Actor, addr string, actType exec.ExecutableActor) *iface.ActorView {
	var actorType string
	var exports iface.ReadableExports
	if actType == nil {
...
func makeReadable(f *exec.FunctionSignature) *iface.ReadableFunctionSignature {
	rfs := &iface.ReadableFunctionSignature{
...
func presentExports(e exec.Exports) iface.ReadableExports {
	rdx := make(iface.ReadableExports)
	for k, v := range e {
