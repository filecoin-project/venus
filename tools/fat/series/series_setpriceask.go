...
// canceled.
func SetPriceGetAsk(ctx context.Context, miner *fat.Filecoin, price *big.Float, expiry *big.Int) (api.Ask, error) {
	// Set a price
	pinfo, err := miner.MinerSetPrice(ctx, price, expiry, fat.AOPrice(big.NewFloat(1.0)), fat.AOLimit(300))
	if err != nil {
		return api.Ask{}, err
	}
