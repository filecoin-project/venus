...
function Usage() {
  echo "USAGE:"
  echo
  echo "makeAsk <REPO_DIR> <MINER_ADDRESS> [ASK_SIZE] [ASK_PRICE] [MSG_TIMEOUT]"
}
...
MSG_CID=$(go-filecoin --repodir="$REPO_DIR" miner add-ask "$MINER_ADDRESS" "$ASK_SIZE" "$ASK_PRICE" --from="$MINER_OWNER")

MSG_RESULT=$(timeout $MSG_TIMEOUT go-filecoin --repodir="$REPO_DIR" message wait $MSG_CID)

EXIT_CODE=$(echo $MSG_RESULT | jq ".exitCode" | grep -v "null")

if [ "$EXIT_CODE" -eq "0" ]; then
   printf "Ask create success\n";
   exit;
else
