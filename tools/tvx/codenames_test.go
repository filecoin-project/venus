...
func TestProtocolCodenames(t *testing.T) {
	...
	if height := abi.ChainEpoch(fork.UpgradeBreezeHeight + 1); GetProtocolCodename(height) != "breeze" {
		t.Fatal("expected breeze codename")
	}

	if height := fork.UpgradeActorsV2Height + 1; GetProtocolCodename(height) != "actorsv2" {
