...
func TestTipSetKey(t *testing.T) {
	...
	t.Run("drops duplicates", func(t *testing.T) {
		...
		for _, cs := range cases {
			cidSet := asSet(cs)
			key := block.NewTipSetKey(cs...)
			assert.Equal(t, len(cidSet), key.Len())
			assert.Equal(t, cidSet, asSet(key.ToSlice()))
			assert.Equal(t, key, block.NewTipSetKey(asSlice(cidSet)...))
		}
	})

	t.Run("fails if unexpected duplicates", func(t *testing.T) {
		_, e := block.NewTipSetKeyFromUnique(c1, c2, c3)
		assert.NoError(t, e)
		_, e = block.NewTipSetKeyFromUnique(c1, c2, c1, c3)
		assert.Error(t, e)
	})

	t.Run("contains", func(t *testing.T) {
		empty := block.NewTipSetKey()
		s := block.NewTipSetKey(c1, c2, c3)
		...
		assert.True(t, s.ContainsAll(block.NewTipSetKey(c1)))
		assert.True(t, s.ContainsAll(s))
		assert.False(t, s.ContainsAll(block.NewTipSetKey(c4)))
		assert.False(t, s.ContainsAll(block.NewTipSetKey(c1, c4)))
...
func TestTipSetKeyCborRoundtrip(t *testing.T) {
	...
	exp := block.NewTipSetKey(makeCid(), makeCid(), makeCid())
	buf, err := cbor.DumpObject(exp)
	assert.NoError(t, err)

	var act block.TipSetKey
