...
import (
	...
	ks "github.com/filecoin-project/go-filecoin/keystore"
...
func MutexWrap(k ks.Keystore) *MutexKeystore {
	return &MutexKeystore{child: k}
}

// Children implements Shim
func (mk *MutexKeystore) Children() []ks.Keystore {
	return []ks.Keystore{mk.child}
}

// Put implements Keystore.Put
func (mk *MutexKeystore) Put(k string, v ci.PrivKey) error {
	mk.Lock()
	defer mk.Unlock()
	return mk.child.Put(k, v)
}

// Get implements Keystore.Get
func (mk *MutexKeystore) Get(k string) (ci.PrivKey, error) {
	mk.RLock()
	defer mk.RUnlock()
