...
type BlockSyntaxValidator interface {
	ValidateSyntax(ctx context.Context, ts types.TipSet) error
}

type BlockSemanticValidator interface {
	ValidateSemantics(ctx context.Context, cur, parent types.TipSet) error
}

type BlockStateValidator interface {
	ValidateState(ctx context.Context, st state.Tree, cur, parent types.TipSet) error
}

type BlockValidatorClock interface {
	Now() types.Uint64
	BlockTime() types.Uint64
}

type ExpectedBlockValidatorClock struct {
	blockTime time.Duration
}

func NewExpectedBlockValidatorClock(blockTime time.Duration) BlockValidatorClock {
	return &ExpectedBlockValidatorClock{
		blockTime: blockTime,
	}
}

func (ebvc *ExpectedBlockValidatorClock) Now() types.Uint64 {
	return types.Uint64(time.Now().Unix())
}

func (ebvc *ExpectedBlockValidatorClock) BlockTime() types.Uint64 {
	return types.Uint64(ebvc.blockTime.Seconds())
}

type ExpectedBlockValidator struct {
	BlockValidatorClock
}

func NewExpectedBlockValidator(clock BlockValidatorClock) BlockValidator {
	return &ExpectedBlockValidator{
		BlockValidatorClock: clock,
	}
}

// ValidateSyntax verifies that this block, on its own, is structurally and
...
func (ebv *ExpectedBlockValidator) ValidateSyntax(ctx context.Context, ts types.TipSet) error {
	// TODO: validate signature on block
	// TODO: valudate fileds
	blks := ts.ToSlice()
	one := blks[0]
	for _, b := range blks {
		...
		if b.Timestamp > ebv.Now() {
			return errors.Errorf("block: %s was generated by miner: %s too far in the future", b.Cid(), b.Miner.String())
		}
		// validate same parents
		if b.Parents.String() != one.Parents.String() {
			return errors.Errorf("invalid parents for block: %s parents: %d & block: %s, parents: %d",
				b.Cid().String(), b.Parents.String(), one.Cid().String(), one.Parents.String())
		}

		// validate same height
		if b.Height != one.Height {
			return errors.Errorf("invalid height for block: %s height: %d & block: %s, height: %d",
				b.Cid().String(), b.Height, one.Cid().String(), one.Height)
		}
	}
	return nil
}

func (ebv *ExpectedBlockValidator) ValidateSemantics(ctx context.Context, current, parent types.TipSet) error {
	pmin, err := parent.MinTimestamp()
	if err != nil {
		return err
	}
	...
	for _, b := range current {
		// validate block was not mined too soon after parent
		// TODO implement tickets from spec
		numNulBlks := uint64(b.Height) - uint64(ph)
		limit := uint64(pmin) + uint64(ebv.BlockTime())*numNulBlks
		if int64(b.Timestamp) < int64(limit) {
			// DONOTMERGE clean up error message before merging, right now its nice to debug with
			return errors.Errorf("block: %s was generated at %d by miner: %s too soon. num null blocks: %d, blocktime: %d parentTs: %d", b.Cid(), b.Timestamp, b.Miner.String(), numNulBlks, ebv.BlockTime(), pmin)
		}
	}
	return nil
}

func (rbv *ExpectedBlockValidator) ValidateState(ctx context.Context, st state.Tree, cur, parent types.TipSet) error {
	return nil
}

// NewExpected is the constructor for the Expected consenus.Protocol module.
func NewExpected(cs *hamt.CborIpldStore, bs blockstore.Blockstore, bv BlockValidator, processor Processor, pt PowerTableView, gCid cid.Cid, verifier proofs.Verifier) Protocol {
	return &Expected{
		cstore:         cs,
		bstore:         bs,
		blockValidator: bv,
...
func (c *Expected) NewValidTipSet(ctx context.Context, blks []*types.Block) (types.TipSet, error) {
	newTs, err := types.NewTipSet(blks...)
	if err != nil {
		return nil, err
	}

	if err := c.blockValidator.ValidateSyntax(ctx, newTs); err != nil {
		return nil, err
	}
	return newTs, nil
}
...
func (c *Expected) RunStateTransition(ctx context.Context, ts types.TipSet, ancestors []types.TipSet, pSt state.Tree) (st state.Tree, err error) {
	...
	if err := c.validateMining(ctx, st, ts, ancestors[0]); err != nil {
		return nil, err
	}
...
func (c *Expected) validateMining(ctx context.Context, st state.Tree, ts types.TipSet, parentTs types.TipSet) error {
	for _, blk := range ts.ToSlice() {
		...
		// block signature is a syntax validation
