...
type TicketValidator interface {
	ValidateTicket(types.Ticket, types.Ticket, address.Address, uint64) (bool, error)
}
...
func (c *Expected) validateMining(ctx context.Context, st state.Tree, ts types.TipSet, parentTs types.TipSet) error {
	...
	for i := 0; i < ts.Len(); i++ {
		...
		// TODO #2223 allow multiticket arrays and check ticket array
		// length against nullblock count
		if len(blk.Tickets) != 1 {
			return errors.New("invalid ticket array length")
		}
		result, err = c.ValidateTicket(prevTicket, blk.Tickets[0], workerAddr, uint64(blk.Height)-prevHeight-1)
		if err != nil {
			return errors.Wrap(err, "failed checking ticket array")
		}
		if !result {
			return errors.New("invalid ticket extension")
		}
