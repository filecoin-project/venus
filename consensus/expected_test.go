...
func TestNewExpected(t *testing.T) {
	...
	t.Run("a new Expected can be created", func(t *testing.T) {
		...
		exp := consensus.NewExpected(cst, bstore, consensus.NewDefaultProcessor(), th.NewFakeBlockValidator(), ptv, someCid, verifier, th.BlockTimeTest)
		assert.NotNil(t, exp)
	})
}
...
func TestExpected_RunStateTransition_validateMining(t *testing.T) {
	...
	t.Run("returns nil + mining error when IsWinningTicket fails due to miner power error", func(t *testing.T) {
		...
		someCid, err := types.CidFromString("somecid")
		assert.NoError(t, err)
		exp := consensus.NewExpected(cistore, bstore, consensus.NewDefaultProcessor(), th.NewFakeBlockValidator(), ptv, someCid, verifier, th.BlockTimeTest)
