...
var initCmd = &cmds.Command{
	...
	Run: func(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) error {
		...
		if err := ensureSectorStorageDirAndMetadataFiles(rep); err != nil {
			return err
		}
...
func ensureSectorStorageDirAndMetadataFiles(r repo.Repo) error {
	metaPath := func(root string) string {
		return filepath.Join(root, stores.MetaFile)
	}
	...
	// ensure that the directory into which we store sector files exists and
	// that it contains a metadata file (a JSON-encoded stores.LocalStorageMeta)
	rdp, err := paths.GetSectorPath(cfg.SectorBase.RootDirPath, rpt)
	if err != nil {
		return err
	}

	_, err = os.Stat(metaPath(rdp))
	if os.IsNotExist(err) {
		_ = os.MkdirAll(rdp, os.ModeDir) // likely that the dir already exists
		...
		if err := ioutil.WriteFile(metaPath(rdp), b, 0644); err != nil {
			return err
		}
	} else if err != nil {
		return err
	...
	// ensure that a pre-sealed sector directory includes the required metadata
	// file (generated by lotus-seed), returning an error if not
	if cfg.SectorBase.PreSealedSectorsDirPath != "" {
		_, err := os.Stat(metaPath(cfg.SectorBase.PreSealedSectorsDirPath))
		if os.IsNotExist(err) {
			return errors.New(fmt.Sprintf("pre-sealed sector directory (%s) does not contain required metadata file (%s)", cfg.SectorBase.PreSealedSectorsDirPath, stores.MetaFile))
		}
		if err != nil {
			return err
		}
