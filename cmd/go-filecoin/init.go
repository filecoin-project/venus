...
var initCmd = &cmds.Command{
	...
	Run: func(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) error {
		...
		if err := ensurePreSealedSectorMetadata(rep); err != nil {
			return err
		}

		if err := ensureSectorStorageMetadata(rep); err != nil {
			return err
		}
...
// file (generated by lotus-seed) and return an error if it does not. This
// function is a no-op if the node is not initialized with a pre-sealed sector directory.
func ensurePreSealedSectorMetadata(r repo.Repo) error {
	cfg := r.Config()

	if cfg.SectorBase.PreSealedSectorsDirPath != "" {
		_, err := os.Stat(filepath.Join(cfg.SectorBase.PreSealedSectorsDirPath, stores.MetaFile))
...
// that it contains a metadata file (a JSON-encoded stores.LocalStorageMeta). If
// the sector path doesn't contain the metadata file, create one.
func ensureSectorStorageMetadata(r repo.Repo) error {
	cfg := r.Config()
	...
	_, err = os.Stat(filepath.Join(rdp, stores.MetaFile))
	if os.IsNotExist(err) {
		...
		if err := ioutil.WriteFile(filepath.Join(rdp, stores.MetaFile), b, 0644); err != nil {
			return err
		}
	} else if err != nil {
		return err
