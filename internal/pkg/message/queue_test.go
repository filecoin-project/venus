...
func TestMessageQueue(t *testing.T) {
	...
	requireRequeue := func(q *message.Queue, msg *types.SignedMessage, stamp uint64) {
		err := q.Requeue(ctx, msg, stamp)
		require.NoError(t, err)
	}

	requireRemoveNext := func(q *message.Queue, sender address.Address, expectedNonce uint64) *types.SignedMessage {
