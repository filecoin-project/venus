...
type ElectionValidator interface {
	...
	VerifyElectionProof(ctx context.Context, entry *drand.Entry, epoch abi.ChainEpoch, miner address.Address, workerSigner address.Address, vrfProof abi.PoStRandomness) error
}
...
func (c *Expected) validateMining(ctx context.Context,
	...
	for i := 0; i < ts.Len(); i++ {
		...
		// TODO Validate ElectionProof
		// TODO check that ElectionProof IsWInner
		...
		_, _, _ = sectorNum, networkPower, sectorSize
