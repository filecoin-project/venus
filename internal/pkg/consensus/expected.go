...
type ElectionValidator interface {
	IsElectionWinner(context.Context, PowerTableView, block.Ticket, block.VRFPi, address.Address, address.Address) (bool, error)
}
...
func (c *Expected) validateMining(ctx context.Context, st state.Tree, ts block.TipSet, parentTs block.TipSet, blsMsgs [][]*types.UnsignedMessage, secpMsgs [][]*types.SignedMessage) error {
	...
	for i := 0; i < ts.Len(); i++ {
		...
		prevTickets := append([]block.Ticket{prevTicket}, blk.Tickets[:len(blk.Tickets)-1]...)
