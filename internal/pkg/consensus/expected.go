...
func (c *Expected) validateMining(ctx context.Context,
	...
	for i := 0; i < ts.Len(); i++ {
		...
		if err := c.VerifyEPoStVrfProof(ctx, blk.Parents, blk.Height-1, blk.Miner, workerSignerAddr, blk.EPoStInfo.VRFProof); err != nil {
		...
		if err := c.IsValidTicket(ctx, blk.Parents, blk.Height-1, blk.Miner, workerSignerAddr, blk.Ticket); err != nil {
