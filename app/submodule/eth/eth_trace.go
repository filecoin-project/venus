...
func buildTraces(ctx context.Context, traces *[]*types.EthTrace, parent *types.EthTrace, addr []int, et types.ExecutionTrace, height int64, ca v1.IChain) error {
		...
		if builtinactors.IsEvmActor(parent.Action.FilecoinCodeCid) {
			...
			if parent.LastByteCode != nil && trace.Action.From == trace.Action.To &&
				...
				if prev.Action.From == trace.Action.From && prev.Action.FilecoinMethod == builtin.MethodsEVM.GetBytecode && prev.Parent == trace.Parent {
					...
					if err != nil {
						return fmt.Errorf("failed UnmarshalCBOR: %w", err)
					}
					trace.Action.Input = dp.Input

					trace.Result.Output, err = decodePayload(et.MsgRct.Return, et.MsgRct.ReturnCodec)
					if err != nil {
						return fmt.Errorf("failed decodePayload: %w", err)
					}
				}
			} else {
	...
	*traces = append(*traces, trace)
	...
	for i, call := range et.Subcalls {
		err := buildTraces(ctx, traces, trace, append(addr, i), call, height, ca)
		if err != nil {
			return err
		}
	}
...
func handleFilecoinMethodInput(method abi.MethodNum, codec uint64, params []byte) ([]byte, error) {
	nativeMethodSelector := []byte{0x86, 0x8e, 0x10, 0xc4}
	evmWordSize := 32

	staticArgs := []uint64{
		uint64(method),
		codec,
		uint64(evmWordSize) * 3,
		uint64(len(params)),
	}
	totalWords := len(staticArgs) + (len(params) / evmWordSize)
	if len(params)%evmWordSize != 0 {
		totalWords++
	}
	len := 4 + totalWords*evmWordSize

	w := &bytes.Buffer{}
	err := binary.Write(w, binary.BigEndian, nativeMethodSelector)
	if err != nil {
