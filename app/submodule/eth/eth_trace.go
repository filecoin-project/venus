...
func decodePayload(payload []byte, codec uint64) (types.EthBytes, error) {
	...
	case multicodec.DagCbor, multicodec.Cbor:
		...
		if err != nil {
			return nil, fmt.Errorf("decodePayload: failed to decode cbor payload: %w", err)
		}
		return buf, nil
	case multicodec.Raw:
		return types.EthBytes(payload), nil
	}

	return nil, fmt.Errorf("decodePayload: unsupported codec: %d", codec)
}
...
func decodeParams[P any, T interface {
	*P
	...
	default:
		return nil, fmt.Errorf("method called with unexpected codec %d", msg.ParamsCodec)
	}
...
func decodeReturn[R any, T interface {
	*R
	...
	case uint64(multicodec.DagCbor), uint64(multicodec.Cbor):
	default:
		return nil, fmt.Errorf("method returned an unexpected codec %d", ret.ReturnCodec)
	}
...
func traceToAddress(act *types.ActorTrace) types.EthAddress {
	...
	return types.EthAddressFromActorID(act.Id)
}
...
func buildTraces(env *environment, addr []int, et *types.ExecutionTrace) error {
	...
	if err != nil {
		return fmt.Errorf("at trace %v: %w", addr, err)
	}
...
func buildTrace(env *environment, addr []int, et *types.ExecutionTrace) (*types.EthTrace, *types.ExecutionTrace, error) {
...
func traceCall(env *environment, addr []int, et *types.ExecutionTrace, input, output types.EthBytes) *types.EthTrace {
	to := traceToAddress(et.InvokedActor)
	callType := "call"
	if et.Msg.ReadOnly {
		callType = "staticcall"
	}
	return &types.EthTrace{
		Type: "call",
		Action: &types.EthCallTraceAction{
			CallType: callType,
			From:     env.caller,
			To:       to,
			Gas:      types.EthUint64(et.Msg.GasLimit),
			Value:    types.EthBigInt(et.Msg.Value),
			Input:    input,
		},
		Result: &types.EthCallTraceResult{
			GasUsed: types.EthUint64(et.SumGas().TotalGas),
			Output:  output,
		},
		TraceAddress: addr,
...
func traceNativeCall(env *environment, addr []int, et *types.ExecutionTrace) *types.EthTrace {
...
func traceEVMCall(env *environment, addr []int, et *types.ExecutionTrace) (*types.EthTrace, *types.ExecutionTrace, error) {
...
func traceNativeCreate(env *environment, addr []int, et *types.ExecutionTrace) (*types.EthTrace, *types.ExecutionTrace, error) {
	if et.Msg.ReadOnly {
	...
	var createdAddr *types.EthAddress
	if et.MsgRct.ExitCode.IsSuccess() {
	...
	return &types.EthTrace{
		Type: "create",
		Action: &types.EthCreateTraceAction{
			From:  env.caller,
			Gas:   types.EthUint64(et.Msg.GasLimit),
			Value: types.EthBigInt(et.Msg.Value),
		...
		Result: &types.EthCreateTraceResult{
			GasUsed: types.EthUint64(et.SumGas().TotalGas),
			Address: createdAddr,
			Code:    output,
		},
...
func decodeCreateViaEAM(et *types.ExecutionTrace) (initcode []byte, addr *types.EthAddress, err error) {
	switch et.Msg.Method {
...
func traceEthCreate(env *environment, addr []int, et *types.ExecutionTrace) (*types.EthTrace, *types.ExecutionTrace, error) {
	...
	if subTrace == nil {
		if et.MsgRct.ExitCode.IsSuccess() {
			return nil, nil, fmt.Errorf("successful Create/Create2 call failed to call a constructor")
		}
		return nil, nil, nil
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("EAM called with invalid params or returned an invalid result, but it still tried to construct the contract: %w", err)
	}
...
func traceEVMPrivate(env *environment, addr []int, et *types.ExecutionTrace) (*types.EthTrace, *types.ExecutionTrace, error) {
	...
	case builtin.MethodsEVM.InvokeContractDelegate:
		...
		if env.lastByteCode == nil {
			return nil, nil, fmt.Errorf("unknown bytecode for delegate call")
		}

		if to := traceToAddress(et.InvokedActor); env.caller != to {
			return nil, nil, fmt.Errorf("delegate-call not from & to self: %s != %s", env.caller, to)
		}

		dp, err := decodeParams[evm12.DelegateCallParams](&et.Msg)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to decode delegate-call params: %w", err)
		}

		output, err := decodePayload(et.MsgRct.Return, et.MsgRct.ReturnCodec)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to decode delegate-call return: %w", err)
		}
