...
var log = logging.Logger("actor_event")
...
var _ v1api.IActorEvent = (*ActorEventHandler)(nil)
...
func (a *ActorEventHandler) parseFilter(f types.ActorEventFilter) (*filterParams, error) {
	...
	min, max, err := parseHeightRange(a.chain.GetHead().Height(), f.FromHeight, f.ToHeight, a.maxFilterHeightRange)
	if err != nil {
		return nil, err
	}
...
func (a *ActorEventHandler) SubscribeActorEventsRaw(ctx context.Context, evtFilter *types.ActorEventFilter) (<-chan *types.ActorEvent, error) {
	...
	minBacklogHeight := a.chain.GetHead().Height() - 1
		...
		for ctx.Err() == nil {
			if len(buffer) > 0 {
				...
				case <-ctx.Done():
					return
				case <-ticker.C:
					currentHeight := a.chain.GetHead().Height()
