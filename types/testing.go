...
func NewMockSigner(kis []KeyInfo) MockSigner {
	...
	for _, k := range kis {
		...
		newAddr, err := address.NewFromActor(address.Mainnet, addrHash)
		if err != nil {
			panic(err)
		}
...
func NewSignedMessageForTestGetter(ms MockSigner) func() *SignedMessage {
	...
	return func() *SignedMessage {
		...
		i++
		msgAddr, err := address.NewFromActor(address.Mainnet, []byte(s+"-to"))
		if err != nil {
			panic(err)
		}
		msg := NewMessage(
...
func NewMessageForTestGetter() func() *Message {
	...
	return func() *Message {
		...
		i++
		fromAddr, err := address.NewFromActor(address.Mainnet, []byte(s+"-from"))
		if err != nil {
			panic(err)
		}
		toAddr, err := address.NewFromActor(address.Mainnet, []byte(s+"-to"))
		if err != nil {
			panic(err)
		}
		return NewMessage(
