...
// MinTicket returns the smallest timestamp of all blocks in the tipset.
func (ts TipSet) MinTimestamp() (uint64, error) {
	if len(ts) == 0 {
		return 0, ErrEmptyTipSet
	}
	blks := ts.ToSlice()
	min := blks[0].Timestamp
	for i := range blks[0:] {
		if blks[i].Timestamp < min {
			min = blks[i].Timestamp
		}
	}
